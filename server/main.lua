...
RegisterCommand('atm', function(source)
    ...
    local masters = xPlayer.Functions.GetItemsByName('bank_card')
    local cards = {}
    ...
    TriggerClientEvent('msfw-atms:client:loadATM', src, cards)
end)
end
-- Event

RegisterNetEvent('msfw-atms:server:enteratm',function ()
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
    local visas = xPlayer.Functions.GetItemsByName('visa')
    local masters = xPlayer.Functions.GetItemsByName('bank_card')
...
RegisterNetEvent('msfw-atms:server:doAccountWithdraw', function(data)
    if data ~= nil then
        local src = source
        local xPlayer = QBCore.Functions.GetPlayer(src)
        ...
        if dailyWith < Config.DailyLimit then
            ...
            TriggerClientEvent('msfw-atms:client:updateBankInformation', src, banking)
        else
...
QBCore.Functions.CreateCallback('msfw-debitcard:server:requestCards', function(source, cb)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
    local visas = xPlayer.Functions.GetItemsByName('visa')
    local masters = xPlayer.Functions.GetItemsByName('bank_card')
...
QBCore.Functions.CreateCallback('msfw-debitcard:server:deleteCard', function(source, cb, data)
...
QBCore.Functions.CreateCallback('msfw-atms:server:loadBankAccount', function(source, cb, cid, cardnumber)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
