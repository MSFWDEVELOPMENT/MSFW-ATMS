...
RegisterNetEvent('msfw-atms:client:updateBankInformation', function(banking)
    SendNUIMessage({
...
if Config.UseTarget then
    CreateThread(function()
        exports['qb-target']:AddTargetModel(Config.ATMModels, {
            options = {
                {
                    event = 'msfw-atms:server:enteratm',
                    type = 'server',
...
RegisterNetEvent('msfw-atms:client:loadATM', function(cards)
...
RegisterNUICallback("doATMWithdraw", function(data)
    if data then
        TriggerServerEvent('msfw-atms:server:doAccountWithdraw', data)
    end
end)

RegisterNUICallback("loadBankingAccount", function(data)
    QBCore.Functions.TriggerCallback('msfw-atms:server:loadBankAccount', function(banking)
...
RegisterNUICallback("removeCard", function(data)
    QBCore.Functions.TriggerCallback('msfw-debitcard:server:deleteCard', function(hasDeleted)
        if hasDeleted then
            SetNuiFocus(false, false)
            SendNUIMessage({
                status = "closeATM"
